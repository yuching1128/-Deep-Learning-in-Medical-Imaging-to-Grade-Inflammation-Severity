...
classifier.add(Dropout(0.5))         
# Softmax 
classifier.add(Dense(units =3, activation = 'softmax'))    # units = 3 for identifing the severity of biceps
# Compiling the CNN
...
classifier.load_weights("graph_S226.h5")
...
file_path = 'C:/Users/ESA.DESKTOP-NTEFSD9/.spyder-py3/data_predict/single/'
...
for i in range(len(f_names)):
    ...
    if f_result[0][1]>max_p:
        ans=1   # moderate
        max_p=f_result[0][1]
    if f_result[0][2]>max_p:
        ans=2   # severe
        max_p=f_result[0][2]
        
	# ans write into a txt file 
    f = open('C:/Users/ESA.DESKTOP-NTEFSD9/Desktop/result/result.txt', 'w')   
    f.write(str(ans))
    f.close()
